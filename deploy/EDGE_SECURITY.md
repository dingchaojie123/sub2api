...
Corgi supports long-lived SSE and WebSocket requests. Protect the request
...
CIDR/IP addresses that connect directly to Corgi. An explicit empty list
...
Corgi `server.trusted_proxies` with the Caddy address/private subnet so the
