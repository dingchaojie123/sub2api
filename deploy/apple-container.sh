...
Commands:
  ...
  up [--recreate]       Create and start the complete Corgi stack
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another Corgi Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
start_app() {
    ...
    if ! wait_for_probe "Corgi" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "Corgi did not become ready."
    fi
    if ! wait_for_probe "Corgi host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "Corgi is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "Corgi stack stopped; persistent volumes were preserved."
}
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the Corgi stack and all persistent data? [y/N] '
    else
        printf 'Delete the Corgi containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "Corgi stack and persistent data deleted."
    else
        info "Corgi stack deleted; persistent volumes were preserved."
    fi
}
