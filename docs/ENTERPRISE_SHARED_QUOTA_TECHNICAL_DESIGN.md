...
| 核心决策 | 员工独立登录；企业统一订阅；文本、图片、视频跨模型、跨分组共享一个总额度池 |
...
| 内容访问 | 成员默认仅查看自己的内容；企业管理员可看团队计费统计，不默认查看他人提示词和生成文件 |

企业总额度按周期控制，第一版不另加日/周额度池。RPM、执行并发、排队长度、单请求费用上限是独立风控配置，不产生额外的消费账本。

### 1.3 本版交付

- 平台后台：企业开通与停用、负责人指定、企业套餐配置、人工授权、账单查询。
...
不包含部门层级、项目预算、SSO/通讯录同步、企业内容共享库、自助退款、额度加购、中途套餐升级、多币种额度池。账户原有登录方式继续复用。
...
- 订阅到期或额度耗尽但成员仍有效时，允许查询自己的已受理任务和下载现有结果。
- 所有记录查询同时限定企业、操作者及角色范围；按任务 ID、导出 ID 或下载 ID 单独查询必须重复执行对象授权。
...
会话、消息、资产和计费操作必须同时满足企业与创建用户归属，成员删除会话/资产只是隐藏或触发内容清理，不删除消费记录。会话不能直接移动到另一企业；复制内容到另一企业需要新建会话，且不能自动沿用无权访问的原企业附件。
...
所有事务固定锁顺序：企业、成员关系、订阅、额度周期、成员用量、操作/预占。单纯校验状态使用兼容的共享锁，状态变更与额度修改使用适当排他锁。执行期间不持有数据库事务。成员停用等管理事务遵循相同顺序，使停用与预占有明确先后关系。
...
| 页面 | 内容与约束 |
| --- | --- |
| 空间选择 | 个人空间和当前可用企业；每个标签页独立；所有收费入口展示付款企业 |
...
- 变体、放大、重绘等按钮仅对支持且属于当前用户的结果开放。后端保存供应商后续操作所需任务 ID/动作 ID；前端提交本地资产或操作 ID，不能把上游控制标识当作授权。
...
团队管理员可以查看成员名称、模型、时间、消费及结果状态，不展示提示词、附件、生成内容或可访问他人内容的上游 URL。导出列遵循同样权限，单次范围和并发受限，下载重新授权；CSV 转义公式前缀，防止用户输入被表格软件当作公式。

提醒默认阈值建议为已消费加预占达到 80%、95%、100%，到期前 7 天、1 天。阈值可配置并标明计入预占；同一企业、周期、阈值只通知一次，释放预占后反复跨线不刷屏。收件人为负责人及明确配置的通知对象，通过后台任务投递，不阻塞生成。

审计覆盖企业创建/停用、邀请、成员恢复/移除、角色变更、限额变更、负责人转移、支付履约、人工授权、退款补偿和导出。日志不记录密码、令牌、完整请求内容或上游密钥。
...
| 1. 企业基础 | schema/迁移、企业、成员、邀请、权限中间件、后台管理 | `backend/ent/schema/`、`backend/migrations/`、`backend/internal/server/routes/`、新增组织服务与 repository |
...
| 6. 网站闭环 | 新增聊天/图片/视频/任务中心、会话与资产存储，以及空间切换、企业管理、用量和导出 | `frontend/src/api/`、`stores/`、`views/workspace/`、`views/organization/`、新增 workspace 后端服务 |
...
1. 新增企业表、可空归属字段和索引；历史个人数据不绑定企业、不转移余额。迁移编号以开发时仓库最新编号为准，不在方案中预占。
...
| 企业隔离 | 猜测企业/成员/任务/文件/导出 ID，均不能访问其他企业或其他成员内容 |
...
| 权限与内容 | 管理员可看团队费用，默认无法读他人提示词和文件；下载过期与撤权边界符合设计 |
...
核心架构已按用户确定方向设计：新增原生网站创作工作台、独立企业订阅、跨全部授权能力的统一额度池，以及企业/成员联合预占和结算。用户已明确网站创作页面尚不存在，已纳入第 5.4、10.4、11 节的数据、接口和交互设计。

实施前仍需完成以下核实，属于具体接入条件，不改变统一额度池方向：
