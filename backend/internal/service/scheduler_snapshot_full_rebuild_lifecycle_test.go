...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Equal(t, schedulerSnapshotAccountQueryCount()*3, accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, schedulerSnapshotAccountQueryCount()*2+1, accounts.callCount())
	require.Equal(t, schedulerSnapshotAccountQueryCount()+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.NoError(t, svc.rebuildFullSnapshot(context.Background(), "test"))
	require.Equal(t, len(schedulerCanonicalBuckets(0))+len(schedulerBucketsForGroup(groupID))+3, cache.captureAttemptCount())
	require.Equal(t, schedulerSnapshotAccountQueryCount()*2+3, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, schedulerSnapshotAccountQueryCount(), accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Zero(t, accounts.callCount())
	require.Zero(t, cache.totalSetAttempts())
	retiredPerInactiveGroup := len(schedulerBucketsForGroup(1)) + 1
	require.Equal(t, retiredPerInactiveGroup, len(cache.retiredBuckets()))
	...
	require.Equal(t, retiredPerInactiveGroup*3, len(cache.retiredBuckets()))
	require.Equal(t, schedulerSnapshotAccountQueryCount(), accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Equal(t, schedulerSnapshotAccountQueryCount()+len(registered), accounts.callCount())
	require.Equal(t, schedulerSnapshotAccountQueryCount()+len(registered), accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, schedulerSnapshotAccountQueryCount()*2, accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, schedulerSnapshotAccountQueryCount()*4, accounts.callCount())
