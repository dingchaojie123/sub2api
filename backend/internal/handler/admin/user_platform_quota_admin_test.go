...
func TestUpdateUserPlatformQuotas_Success(t *testing.T) {
	...
	body := `{"quotas":[
		...
		{"platform":"midjourney","daily_limit_usd":null,"weekly_limit_usd":null,"monthly_limit_usd":null},
		{"platform":"kling","daily_limit_usd":null,"weekly_limit_usd":null,"monthly_limit_usd":null},
		{"platform":"happyhourse","daily_limit_usd":null,"weekly_limit_usd":null,"monthly_limit_usd":null},
	...
	if len(cache.deleteCalls) != len(service.AllowedQuotaPlatforms) {
		t.Errorf(
			"expected %d cache delete calls, got %d: %+v",
			len(service.AllowedQuotaPlatforms),
			len(cache.deleteCalls),
			cache.deleteCalls,
		)
	}
}
...
func TestUpdateUserPlatformQuotas_RejectsTooManyEntries(t *testing.T) {
	h := buildTestHandler(&upsertCapturingQuotaRepo{}, &billingCacheStub{})
	body := `{"quotas":[
		{"platform":"anthropic"},{"platform":"openai"},{"platform":"gemini"},{"platform":"antigravity"},{"platform":"grok"},{"platform":"jimeng"},{"platform":"anthropic"}
	]}`
	c, w := putReq(t, body)
	h.UpdateUserPlatformQuotas(c)
	if w.Code != http.StatusBadRequest {
